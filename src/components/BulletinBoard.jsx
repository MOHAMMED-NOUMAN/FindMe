...
export default function BulletinBoard() {
  ...
  if (loading) return (
    <div className="px-5 pb-12 max-w-6xl mx-auto flex justify-center">
      <div className="animate-pulse flex gap-2 items-center text-slate-400">Loading notices...</div>
    </div>
  )
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm text-red-600 font-semibold uppercase tracking-wide flex items-center gap-1.5 mt-0.5">
            <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" /> Live Flash Notices
          </p>
        </div>
      </motion.div>
              ...
              <div className="space-y-2 text-sm text-[#475569]">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <span className="line-clamp-2 leading-tight">
                    {person.lastKnownLocation?.description || person.lastKnownLocation?.district || 'Location unverified'}
                    {person.lastKnownLocation?.district && person.lastKnownLocation?.description ? ` (${person.lastKnownLocation.district})` : ''}
                  </span>
                </div>
              </div>
            </div>
