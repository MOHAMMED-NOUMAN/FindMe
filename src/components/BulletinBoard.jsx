...
export default function BulletinBoard() {
  ...
  function getDummyData(activeFilter) {
    const base = [
      { id: "1", name: "Rajan Kumar",  age: 34, lastKnownLocation: { district: "Wayanad",   description: "Near Chooralmala bridge" }, gender: "Male" },
      { id: "2", name: "Aisha Begum",  age: 28, lastKnownLocation: { district: "Ernakulam", description: "Marine Drive area" },        gender: "Female" },
      { id: "3", name: "Kiran",        age: 12, lastKnownLocation: { district: "Idukki",    description: "Munnar town center" },       gender: "Male" },
      { id: "4", name: "Priya",        age: 45, lastKnownLocation: { district: "Thrissur",  description: "Railway Station" },          gender: "Female" },
    ];
    if (activeFilter === "children") return base.filter((p) => (p.age ?? 99) < 18);
    if (activeFilter === "adults")   return base.filter((p) => (p.age ?? 0) >= 18);
    return base;
  }

  const chips = [
    { id: "all",      labelKey: "bulletin.all" },
    { id: "children", labelKey: "bulletin.children" },
    { id: "adults",   labelKey: "bulletin.adults" },
  ];
    ...
    <section className="px-5 pb-16 max-w-6xl mx-auto">
              ...
              <div className="space-y-2 text-sm text-[#475569]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="truncate">
                    {person.age ? `${person.age} yrs` : t("bulletin.unknown_age")}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <span className="line-clamp-2 leading-tight">
                    {person.lastKnownLocation?.description ||
                      ...
                      t("bulletin.location_unverified")}
                    {person.lastKnownLocation?.district &&
                    person.lastKnownLocation?.description
                      ? ` (${person.lastKnownLocation.district})`
                      : ""}
      ...
      {!loading && notices.length === 0 && (
        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
          {t("bulletin.no_notices", {
            filter: filter === "all" ? "" : `${t(`bulletin.${filter}`)} `,
          })}
        </div>
      )}
    </section>
